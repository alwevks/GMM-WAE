...
import decoder as basic_decoder


class StochasticWAEModel(object):

    def __init__(self, config, embeddings_matrix, word_index, idx_genre):
        self.config = config
    ...
    def init_placeholders(self):
        with tf.name_scope("model_inputs"):
            ...
            self.inference_labels = tf.placeholder(tf.float32, [self.num_classes, 1], name='inference_labels')
        ...
        with tf.name_scope("encode"):
            for layer in range(self.num_layers):
                with tf.variable_scope('encoder_{}'.format(layer + 1)):
                    cell_fw = tf.contrib.rnn.LayerNormBasicLSTMCell(self.lstm_hidden_units, dropout_keep_prob=self.keep_prob)

                    cell_bw = tf.contrib.rnn.LayerNormBasicLSTMCell(self.lstm_hidden_units, dropout_keep_prob=self.keep_prob)
    ...
    def sample_gaussian(self):
        with tf.name_scope('sample_gaussian'):
            ...
            self.prior_means = tf.Variable(
                initial_value=np.random.rand(self.num_classes, self.latent_dim),
                dtype=tf.float32, trainable=True)
            self.prior_sigmas = tf.Variable(
                initial_value=np.random.rand(self.num_classes, self.latent_dim),
                dtype=tf.float32, trainable=True)

            m = tf.nn.embedding_lookup(self.prior_means, self.labels)
            s = tf.nn.embedding_lookup(self.prior_sigmas, self.labels)
            epsilon = tf.random_normal([self.batch_size, self.latent_dim], name='z_sampled')
            self.z_sampled = m + epsilon * tf.exp(s)

            epsilon = tf.expand_dims(epsilon, axis=1)
            epsilon = tf.tile(epsilon, [1, self.num_classes, 1])
            m = tf.expand_dims(self.prior_means, axis=0)
            m = tf.tile(m, [self.batch_size, 1, 1])
            s = tf.expand_dims(self.prior_sigmas, axis=0)
            s = tf.tile(s, [self.batch_size, 1, 1])

            self.inference_z_sampled = m + epsilon * tf.exp(s)

            weights = tf.expand_dims(self.inference_labels, axis=0)
            weights = tf.tile(weights, [self.batch_size, 1, self.latent_dim])
            print(m, s, weights, self.inference_z_sampled)

            self.inference_z_sampled = weights * self.inference_z_sampled
            self.inference_z_sampled = tf.reduce_sum(self.inference_z_sampled, axis=1)

    def build_decoder(self):
        with tf.variable_scope("decode"):
            for layer in range(self.num_layers):
                with tf.variable_scope('decoder_{}'.format(layer + 1)):
                    dec_cell = tf.contrib.rnn.LayerNormBasicLSTMCell(2 * self.lstm_hidden_units, dropout_keep_prob=self.keep_prob)
            ...
            with tf.name_scope("validate_decoder"):
                ...
                validate_helper = tf.contrib.seq2seq.GreedyEmbeddingHelper(self.embeddings,
                                                                            start_tokens,
                                                                            end_token)

                validate_decoder = basic_decoder.BasicDecoder(dec_cell,
                                                               validate_helper,
                                                               initial_state=self.init_state,
                                                               latent_vector=self.z_tilda,
                                                               output_layer=self.output_layer)

                self.validate_logits, _state, _len = tf.contrib.seq2seq.dynamic_decode(validate_decoder,
                                                                                        output_time_major=False,
                                                                                        impute_finished=True,
                                                                                        maximum_iterations=self.num_tokens)
            ...
            with tf.name_scope("inference_decoder"):
                ...
                inference_decoder = basic_decoder.BasicDecoder(dec_cell,
                                                               inference_helper,
                                                               initial_state=self.init_state,
                                                               latent_vector=self.inference_z_sampled,
                                                               output_layer=self.output_layer)
    ...
    def monitor(self, x_val, labels_val, sess, epoch_i, time_consumption):
        self.validate(sess, x_val, labels_val)
    ...
    def train(self, x_train, x_val, labels_train, labels_val):
        ...
        with tf.Session() as sess: 
            ...
            for epoch_i in range(1, self.epochs + 1):
                ...
                for batch_i, (input_batch, output_batch, labels_batch, sent_lengths) in enumerate(
                        utils.get_batches(x_train, labels_train, self.batch_size)):

                    try:
                        iter_i += 1
                        _, _summary, self.train_xent, p_means = sess.run(
                            [self.train_op, self.summary_op, self.xent_loss, self.prior_means],
                ...
                self.monitor(x_val, labels_val, sess, epoch_i, time_consumption)
                if epoch_i % 10 == 0 and epoch_i != 0:
                    self.generate_samples(sess, epoch_i)

    def generate_samples(self, sess, epoch_i):
        for i in range(self.num_classes):
            self.generate_samples_with_style(i, sess, epoch_i)
            for j in range(i + 1, self.num_classes):
                self.generate_samples_with_two_styles(i, j, sess, epoch_i)

    def generate_samples_with_style(self, style, sess, epoch_i):
        inference_labels = np.reshape(np.eye(self.num_classes)[style - 1], (self.num_classes, 1))
        samples = []
        for i in range(10):
            result = sess.run(self.inference_logits, feed_dict={self.inference_labels: inference_labels,
                                                            self.keep_prob: 1.0})
        ...
        text_file = open('output/' + self.idx_genre[style] + '_epoch_' + str(epoch_i) + '.txt', 'w')
        text_file.write(generated)
        text_file.close()

    def generate_samples_with_two_styles(self, style1, style2, sess, epoch_i):
        inference_labels = np.reshape(np.eye(self.num_classes)[style1 - 1], (self.num_classes, 1))
        inference_labels += np.reshape(np.eye(self.num_classes)[style2 - 1], (self.num_classes, 1))
        inference_labels /= 2
        samples = []
        for i in range(10):
            result = sess.run(self.inference_logits, feed_dict={self.inference_labels: inference_labels,
        ...
        for pred in samples:
            generated += ' '.join([self.idx_word[i] for i in pred if i not in [self.pad, self.eos]]) + '\n'
        text_file = open('output/' + self.idx_genre[style1] + '_' + self.idx_genre[style2] + '_epoch_' + str(epoch_i) + '.txt', 'w')
        text_file.write(generated)
        text_file.close()

    def validate(self, sess, x_val, labels_val):
        # Calculate BLEU on validation data
        hypotheses_val = []
        references_val = []

        for batch_i, (input_batch, output_batch, labels_batch, sent_lengths) in enumerate(
                utils.get_batches(x_val, labels_val, self.batch_size)):
    ...
    def random_sample(self, checkpoint):
        ...
        with tf.Session() as sess:
            ...
            sample_labels = [0, 1, 2 ,3] * 128

            result = sess.run(self.inference_logits,
                              feed_dict={self.labels: sample_labels,
                                         self.keep_prob: 1.0})
    ...
    def random_sample_in_session(self, sess):
        inference_labels = [[1.], [0], [0], [0]]
        result = sess.run(self.inference_logits, feed_dict={self.inference_labels: inference_labels,
                                                           self.keep_prob: 1.0})

        generated = ''

        for pred in result[:20]:
            generated += '\t\t' + ' '.join([self.idx_word[i] for i in pred if i not in [self.pad, self.eos]]) + '\n'
        return generated
    ...
    def random_sample_save(self, checkpoint, num_batches=1):
        ...
        with tf.Session() as sess:
            ...
            for i in range(num_batches):
                result = sess.run(self.inference_logits,
                                  feed_dict={self.z_sampled: self.prior_dists[0],
                                             self.keep_prob: 1.0})
    ...
    def get_zvector(self, x_test, sess=None, checkpoint=None):
        z_vecs = []
        ...
        for batch_i, (input_batch, output_batch, _, sent_lengths) in enumerate(
                utils.get_batches(x_test, x_test, self.batch_size)):
